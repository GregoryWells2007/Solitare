...
void card_renderer_draw_card(card_renderer* renderer, vector2 position, ivec3 card_data) {
    ...
    struct card_vertex vertices[24] = {
        { (-1.000f * CARD_SIZE) + position.x,  (1.750f * CARD_SIZE) + position.y, 0.0556f, 1.0000f, card_data },
        { (-1.045f * CARD_SIZE) + position.x,  (1.745f * CARD_SIZE) + position.y, 0.0356f, 0.9986f, card_data },
        { (-1.075f * CARD_SIZE) + position.x,  (1.735f * CARD_SIZE) + position.y, 0.0222f, 0.9957f, card_data },
        { (-1.110f * CARD_SIZE) + position.x,  (1.700f * CARD_SIZE) + position.y, 0.0067f, 0.9857f, card_data },
        { (-1.120f * CARD_SIZE) + position.x,  (1.670f * CARD_SIZE) + position.y, 0.0022f, 0.9771f, card_data },
        { (-1.125f * CARD_SIZE) + position.x,  (1.625f * CARD_SIZE) + position.y, 0.0000f, 0.9643f, card_data },
        
        { (-1.125f * CARD_SIZE) + position.x, (-1.625f * CARD_SIZE) + position.y, 0.0000f, 0.0357f, card_data },
        { (-1.120f * CARD_SIZE) + position.x, (-1.670f * CARD_SIZE) + position.y, 0.0022f, 0.0223f, card_data },
        { (-1.110f * CARD_SIZE) + position.x, (-1.700f * CARD_SIZE) + position.y, 0.0067f, 0.0143f, card_data },
        { (-1.075f * CARD_SIZE) + position.x, (-1.735f * CARD_SIZE) + position.y, 0.0222f, 0.0043f, card_data },
        { (-1.045f * CARD_SIZE) + position.x, (-1.745f * CARD_SIZE) + position.y, 0.0356f, 0.0014f, card_data },
        { (-1.009f * CARD_SIZE) + position.x, (-1.750f * CARD_SIZE) + position.y, 0.0556f, 0.0000f, card_data },

        {  (1.000f * CARD_SIZE) + position.x, (-1.750f * CARD_SIZE) + position.y, 0.9444f, 0.0000f, card_data  },
        {  (1.045f * CARD_SIZE) + position.x, (-1.745f * CARD_SIZE) + position.y, 0.9644f, 0.0014f, card_data  },
        {  (1.075f * CARD_SIZE) + position.x, (-1.735f * CARD_SIZE) + position.y, 0.9778f, 0.0043f, card_data  },
        {  (1.110f * CARD_SIZE) + position.x, (-1.700f * CARD_SIZE) + position.y, 0.9933f, 0.0143f, card_data  },
        {  (1.120f * CARD_SIZE) + position.x, (-1.670f * CARD_SIZE) + position.y, 0.9978f, 0.0223f, card_data  },
        {  (1.125f * CARD_SIZE) + position.x, (-1.625f * CARD_SIZE) + position.y, 1.0000f, 0.0357f, card_data  },

        {  (1.125f * CARD_SIZE) + position.x,  (1.625f * CARD_SIZE) + position.y, 1.0000f, 0.9643f, card_data },
        {  (1.120f * CARD_SIZE) + position.x,  (1.670f * CARD_SIZE) + position.y, 0.9978f, 0.9771f, card_data },
        {  (1.110f * CARD_SIZE) + position.x,  (1.700f * CARD_SIZE) + position.y, 0.9933f, 0.9857f, card_data },
        {  (1.075f * CARD_SIZE) + position.x,  (1.735f * CARD_SIZE) + position.y, 0.9778f, 0.9957f, card_data },
        {  (1.045f * CARD_SIZE) + position.x,  (1.745f * CARD_SIZE) + position.y, 0.9644f, 0.9986f, card_data },
        {  (1.000f * CARD_SIZE) + position.x,  (1.750f * CARD_SIZE) + position.y, 0.9444f, 1.0000f, card_data }
    };
