...
void platform_array_buffer_build(array_buffer* buffer) {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(buffer->data[0]) * buffer->vertex_count, buffer->data, (buffer->draw_type == static_draw) ? GL_STATIC_DRAW : GL_DYNAMIC_DRAW);
    ...
    for (int i = 0; i < buffer->attribute_count; i++) {
        ...
        glVertexAttribPointer(i, buffer->attribute_count, type, GL_FALSE, array_size, (const void*)current_offset);
    }
